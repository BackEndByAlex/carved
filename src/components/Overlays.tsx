...
const SKILLS = [
  ...
  { t: "Security & Privacy", d: "OWASP, secure headers, input validation, auth best practices" },
];
...
const AI_TOOLS = [
  {
    name: "GitHub Copilot",
    since: "2024",
    role: "First AI tool. Autocomplete, inline suggestions while coding.",
    tone: "amber",
  },
  {
    name: "ChatGPT",
    since: "2025",
    role: "Debugging, explanations, rubber duck at scale, learning.",
    tone: "cyan",
  },
  {
    name: "Claude",
    since: "2026",
    role: "Deep reasoning, architecture thinking, long-context work, security reviews, code review, and system design.",
    tone: "amber",
  },
  {
    name: "Gemini",
    since: "2026",
    role: "Research, Google ecosystem integration, quick lookups.",
    tone: "cyan",
  },
] as const;
  ...
  return (
    <div className="scene-overlay items-center justify-center px-4 text-center sm:px-6">
      <div className="max-w-2xl" style={withFade(progress, 0)}>
        <h1 className="mt-4 font-display text-4xl font-bold leading-none text-amber-glow sm:mt-5 sm:text-5xl md:text-7xl">
          Alexandru C.A
        </h1>
        <p className="mx-auto mt-4 max-w-xl text-sm text-foreground/75 sm:mt-6 sm:text-base md:text-lg">
          Web Developer Student at Linnaeus University, Sweden.
        </p>
...
export function AboutOverlay({ progress }: { progress: number }) {
  return (
    <div className="scene-overlay items-center px-4 py-14 sm:px-6 sm:py-0">
      <div
        className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 md:grid-cols-2 md:gap-10"
        style={withFade(progress, 1)}
      >
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-amber-glow sm:text-xs">
            // 02 · about
          </p>
          <h2 className="mt-3 font-display text-3xl font-semibold sm:mt-4 md:text-5xl">
            Building things that actually work.
          </h2>
          <p className="mt-4 max-w-md text-sm text-foreground/75 sm:mt-6 sm:text-base">
            Second year at Linnaeus University. I treat every school assignment as a real work
            project — applying best practices, understanding how everything works, and always asking
            why.
          </p>
          <p className="mt-3 max-w-md text-sm text-foreground/60 sm:mt-4">
            Most of my time and coffee goes to researching best practices, finding the best way to
            make projects secure, and a drive to learn how to co-work with AI and get as effective
            as I can.
          </p>
        </div>
...
function Row({ k, v, last }: { k: string; v: React.ReactNode; last?: boolean }) {
  return (
    <div
      className={`flex items-center justify-between gap-4 py-2 ${last ? "" : "border-b border-border/60"}`}
    >
    ...
    <div className="scene-overlay items-center px-4 py-14 sm:px-6 sm:py-0">
      <div className="mx-auto w-full max-w-6xl" style={withFade(progress, 2)}>
        <div className="mb-5 text-center sm:mb-8">
          <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-cyan-glow sm:text-xs">
            // 03 · skills
          </p>
          <h2 className="mt-2 font-display text-3xl font-semibold sm:mt-3 md:text-5xl">
            ...
            >
              ...
              <div className="mt-auto flex flex-wrap gap-1 pt-3">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded border border-border px-2 py-0.5 text-[10px] uppercase tracking-wider text-foreground/60"
                  >
                    {t}
                  </span>
                ))}
...
export function CVOverlay({ progress }: { progress: number }) {
  return (
    <div className="scene-overlay items-center px-4 py-14 sm:px-6 sm:py-0">
      <div
        className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-5 md:grid-cols-2 md:gap-8"
        style={withFade(progress, 4)}
      >
        <div className="rounded-lg border border-amber-glow/40 bg-background/75 p-4 backdrop-blur sm:p-6">
          ...
          <ul className="mt-2 space-y-2 text-xs sm:mt-3 sm:space-y-4 sm:text-sm">
            <li>
              <p className="font-display text-sm text-foreground/90 sm:text-base">
                SI-ledarutbildning
              </p>
        ...
        <div className="rounded-lg border border-cyan-glow/40 bg-background/75 p-4 backdrop-blur sm:p-6">
          ...
          <ul className="mt-2 space-y-1.5 text-xs sm:mt-3 sm:space-y-3 sm:text-sm">
            <li>
              <p className="font-display text-sm text-foreground/90 sm:text-base">
                Teaching Assistant{" "}
                <span className="font-sans text-foreground/50 font-normal">· LNU, Kalmar</span>
              </p>
              <p className="font-mono text-[10px] text-foreground/40 sm:text-xs">
                Sep 2025 → Apr 2026
              </p>
            ...
            <li className="border-t border-border/40 pt-1.5 sm:pt-3">
              <p className="font-display text-sm text-foreground/90 sm:text-base">
                IT-Tekniker{" "}
                <span className="font-sans text-foreground/50 font-normal">
    ...
    <div className="scene-overlay items-center px-4 py-14 sm:px-6 sm:py-0">
      <div className="mx-auto w-full max-w-6xl" style={withFade(progress, 5)}>
        <div className="mb-5 text-center sm:mb-8">
          ...
          <h2 className="mt-2 font-display text-3xl font-semibold sm:mt-3 md:text-5xl">
            Tools I use to become better.
          </h2>
          <p className="mx-auto mt-2 max-w-2xl text-xs text-foreground/65 sm:mt-3 sm:text-sm">
            Each one makes me sharper — I use them to learn faster, think deeper, and grow at what I
            do.
          </p>
        </div>
          ...
          {AI_TOOLS.map((tool) => (
            ...
            >
              <div className="flex items-baseline justify-between">
                ...
                <span
                  className={`font-mono text-[10px] uppercase tracking-wider sm:text-xs ${tool.tone === "amber" ? "text-amber-glow" : "text-cyan-glow"}`}
                >
                  since {tool.since}
                </span>
              </div>
...
const BUILD_STACK = [
  {
    ...
    tone: "amber" as const,
    detail:
      "The 3D cave is built with Three.js via React Three Fiber. Instanced meshes keep rock geometry cheap — 280 icosahedrons rendered as one draw call using a seeded PRNG (mulberry32) for deterministic placement.",
  },
  {
    label: "GSAP · Lenis",
    tone: "cyan" as const,
    detail:
      "Lenis drives smooth-wheel scrolling. GSAP ScrollTrigger pins the canvas and writes raw scroll progress into a plain mutable ref — no React state — so Three.js reads it each frame without triggering re-renders.",
  },
  {
    label: "CatmullRom Spline",
    tone: "amber" as const,
    detail:
      "Six camera waypoints are threaded through a CatmullRomCurve3 spline. Progress 0→1 maps to a smooth flight through the cave. A second spline handles lookAt targets independently for natural camera movement.",
  },
  {
    label: "React 19 · Vite · TS",
    tone: "cyan" as const,
    detail:
      "Plain SPA — no SSR, no server runtime. TanStack Router handles the single route. Tailwind v4 with custom CSS properties for the amber/cyan brand colours. PostProcessing adds Bloom and Vignette over the canvas.",
  },
  {
    label: "Caddy · VPS",
    tone: "amber" as const,
    detail:
      "Deployed to a VPS at backendbyalex.se. Caddy serves the dist/ folder with gzip, security headers, and TLS automatically.",
  },
  {
    label: "Snap-to-scene",
    tone: "cyan" as const,
    detail:
      "After 500ms of scroll idle, a timeout fires and lerps the scroll position to the nearest scene boundary. This gives the feel of a snapping carousel while still allowing free intermediate scrolling.",
  },
];
    ...
    <div className="scene-overlay items-center px-4 py-14 sm:px-6 sm:py-0">
      <div className="mx-auto w-full max-w-6xl" style={withFade(progress, 6)}>
        <div className="mb-5 text-center sm:mb-8">
          ...
          <p className="mx-auto mt-2 max-w-2xl text-xs text-foreground/65 sm:mt-3 sm:text-sm">
            Researched and built with Claude and Gemini — figuring out the best way to put this
            together.
          </p>
        </div>
          ...
          {BUILD_STACK.map((item, i) => (
            ...
            >
              <div className="flex items-baseline justify-between gap-2">
                <span
                  className={`font-mono text-[10px] sm:text-xs ${item.tone === "amber" ? "text-amber-glow" : "text-cyan-glow"}`}
                >
                  0{i + 1}
                </span>
              </div>
              <p className="mt-1 font-display text-base sm:text-lg">{item.label}</p>
              <p className="mt-1.5 text-[11px] leading-relaxed text-foreground/65 sm:text-xs">
                {item.detail}
              </p>
            </li>
          ))}
